...
def create(data: ProductSchemas, db: Session):
    ...
    if data.components:
        for asoc_component in data.components:
            stmt = (
                select(Component).where(Component.id == asoc_component.component_id)
            )
    ...
    if data.semifinisheds:
        for asoc_semifinished in data.semifinisheds:
            stmt = (
                select(SemiFinished).where(SemiFinished.id == asoc_semifinished.semifinished_id)
            )
    ...
    #         stmt = (
    #             select(Product).where(Product.id == asoc_product.product_id)
    #         )
    #
    #         product = db.execute(stmt).scalar() #type: Product
    #
    #         new_product.product_id_list.append(product)
...
def get(id: int, db: Session):
    ...
    stmt = (
        select(Product)
        .where(Product.id == id)
        .options(selectinload(Product.component_list)
                 .options(load_only(ProductComponent.component_count))
                 .joinedload(ProductComponent.component))
        .options(selectinload(Product.semifinished_list)
    ...
    def count_recursive(data, multiple: int=1):
        ...
        if hasattr(data, "component_list"):
            for com_item in data.component_list:
                ...
                result_components[com_item.component_id] = result_components.get(com_item.component_id, 0) + com_item.component_count * multiple

        if hasattr(data, "semifinished_list"):
            for sem_item in data.semifinished_list:
                count_recursive(sem_item.semifinished, sem_item.semifinished_count * multiple)
